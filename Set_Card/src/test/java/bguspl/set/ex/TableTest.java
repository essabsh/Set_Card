...
class TableTest {
    ...
    private void placetokens(){
        table.placeToken(2,0);
        table.placeToken(2,1);
        table.placeToken(2,2);
    }
    private void removeall(){
        for (int i=0 ; i<table.slotToCard.length;i++){
            if (table.playerToSlots.get(2).contains(i))
                table.removeToken(2,i);
        }
    }
    ...
    @Test
    void placetokenswithfilled(){
        try{
            placeCard_AllSlotsAreFilled();
        }catch(InterruptedException ex){}
        placetokens();
        int count = numberoftokens();
        assertEquals(3,count);
    }
    @Test
    void removetokenswithnotokens(){
        placetokens();
        removeall();
        int count =numberoftokens();
